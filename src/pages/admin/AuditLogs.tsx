...
export default function AuditLogs() {
  ...
  const filtered = auditLogs.filter((l) => (action === "all" || l.action === action) && (l.user + l.entity).toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" onClick={() => toast.success("CSV exported")}><Download className="w-4 h-4 mr-2" /> Export</Button>
      </div>

      <div className="flex gap-3">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by user or entity..." className="pl-9" />
        </div>
            ...
            {filtered.map((l) => {
              ...
              return (
                <tr key={l.id} className="border-t border-border hover:bg-muted/30">
                  <td className="p-3 font-mono text-xs whitespace-nowrap">{format(new Date(l.timestamp), "MMM d, HH:mm:ss")}</td>
                  <td className="p-3 font-semibold">{l.user}</td>
                  <td className="p-3"><span className="text-xs text-muted-foreground capitalize">{l.role}</span></td>
                  <td className="p-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold ${colors[l.action]}`}>
